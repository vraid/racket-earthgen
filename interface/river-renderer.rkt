...
(define river-renderer%
  (class object%
    ...
    (define/public (resize-buffer grid)
      (when (resize-buffer? grid)
        (let* ([tile-count (tile-count grid)]
          ...
          (set! buffer buf))))
    (: set-shapes (-> planet-climate (-> FlVector (-> FlVector FlVector)) Void))
    ...
    (: set-colors (-> planet-climate (-> FlVector (-> FlVector FlVector)) Void))
    (define/public (set-colors planet transform)
      (update-buffer planet transform buffer)
      ((gl-buffer-bind buffer)))
    (: render (-> Void))
...
(define (init-buffer planet transform buffer)
